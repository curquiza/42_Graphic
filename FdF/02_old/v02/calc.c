...
/*   Updated: 2017/03/01 10:29:36 by curquiza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_calc_withalti(t_point **map)
{
	int		i;
	int		j;
	...
	while (map[i])
	{
		...
		{
			if (map[i][j].alti)
				map[i][j].pix->y += (-1) * map[i][j].alti * COEF_ALTI;
			j++;
		}
		i++;
	}
...
void		ft_calc_all(t_point **map, t_param *param)
{
	...
	//ft_putendl("APRES CALC SUR PLAT");
	//ft_putmap_coord(map);
	ft_calc_withalti(map);
	//ft_putendl("APRES CALC DES ALTI");
	//ft_putmap_coord(map);
	ft_adjust_coord(map);
	//ft_putendl("APRES AJUSTEMENT");
	//ft_putmap_coord(map);
